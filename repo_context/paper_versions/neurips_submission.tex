...
% ---------------------------------------------------------------------------
\title{Geometric Competition as a Shared Principle of Ambiguity\\
       in LLM and Neural Emotion Representations}

\author{
  Anonymous Authors \\
  \textit{Submitted to NeurIPS 2026 --- Double-blind review version}
}
...
The cross-system context retention experiment (Section~\ref{sec:finding3}) additionally
...
We present four findings in order of increasing cross-system scope.
Findings 1--2 characterise representational geometry and signal structure;
Finding 3 introduces the cross-system ambiguity gradient;
Finding 4 documents the global representational inversion.

% ---------------------------------------------------------------------------
\subsection{Finding 1: Radial Density Structure}
\label{sec:finding1}
% ---------------------------------------------------------------------------
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.44\textwidth}
    \includegraphics[width=\textwidth]{all_clusters_comparison}
    \caption{PCA projections of all 8 LLM variants.
             Pretrained models (rows 1--2): overlapping clouds.
             Fine-tuned models (rows 3--4): compact, separated islands.}
    \label{fig:clusters}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.53\textwidth}
    \includegraphics[width=\textwidth]{global_density_decay_comparison_}
    \caption{Normalised radial density for LLM variants and human brain fMRI (dashed).
             All systems share the same void-belt profile despite substrate differences.}
    \label{fig:density-brain}
  \end{subfigure}
  \caption{\textbf{Radial density structure across LLMs and brain (Finding 1).}
           (a) Fine-tuning sharpens cluster boundaries but does not change the
           underlying radial structure.
           (b) LLMs and brain fMRI share a consistent two-zone density profile:
           zero density near the centroid, a peak-density shell at intermediate
           normalised distance.}
  \label{fig:clusters-density}
\end{figure}

A consistent two-zone density structure emerges across all 8 LLM variants and human
brain fMRI (Figure~\ref{fig:clusters-density}).

\textbf{The Void.}
No data points exist near any emotion centroid.
The Void is a geometric consequence of distributed within-class variance: a centroid
is the mean of a distributed ring of instances and therefore sits inside the ring in
zero-density space.
Fine-tuned models have a shorter void than pretrained models, reflecting that
fine-tuning pulls instances closer toward prototype boundaries (see table below).

\textbf{The Belt.}
Density rises sharply from the void boundary and peaks at a system-dependent normalised
distance that is earlier for fine-tuned models and later for pretrained models.
The human brain exhibits the same profile (Figure~\ref{fig:density-brain}), with its
density peak near normalised distance 1.0, consistent with distributed biological coding.

\textbf{Certainty buffer} (radial gap between density peak and onset of $>\!5\%$
geometric overlap):
...
\midrule
MPNet-FT-Final   & 0.438 & 0.562 & 2.438 & $+1.875$ \\
BGE-FT-Final     & 0.438 & 0.688 & 2.438 & $+1.750$ \\
MPNet-Base-Final & 0.688 & 0.938 & 0.938 & $\phantom{+}0.000$ \\
BGE-Base-Final   & 0.688 & 0.938 & 1.062 & $+0.125$ \\
\bottomrule
\end{tabular}
\end{center}

Fine-tuning creates a large safe zone between peak density and competitive class
confusion; for pretrained models this buffer is near zero.
No embedding aligns exclusively with a single class prototype in any system.
Global overlap ranges from 0.42\% (MPNet-FT-Final) to 19.22\% (BGE-Base-Final),
confirming that fine-tuning substantially reduces inter-class geometric confusion.

% ---------------------------------------------------------------------------
\subsection{Finding 2: Signal Compaction and the Low-Rank Emotion Subspace}
\label{sec:finding2}
% ---------------------------------------------------------------------------
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.55\textwidth}
    \includegraphics[width=\textwidth]{early_decay_comparison_top25}
    \caption{SVD ablation for LLMs: accuracy as dominant directions are removed
             iteratively. Fine-tuned models collapse to chance at dimensions 15--20;
             pretrained BGE at 26, pretrained MPNet at 67.}
    \label{fig:signal-cliff}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.42\textwidth}
    \includegraphics[width=\textwidth]{Content_Retention_Comparison_}
    \caption{Cross-system signal retention: brain ($D_{50}\!=\!4$), fine-tuned LLM
             ($D_{50}\!=\!12$), pretrained LLM ($D_{50}\!=\!17$).
             The brain decays fastest initially but plateaus rather than collapsing.}
    \label{fig:context-retention}
  \end{subfigure}
  \caption{\textbf{Signal compaction across LLMs and brain (Finding 2).}
           (a) Fine-tuning concentrates all emotional signal into 15--20 directions.
           (b) Extending to human brain fMRI reveals qualitatively distinct encoding
           strategies across systems.}
  \label{fig:signal-structure}
\end{figure}

We apply iterative SVD ablation: at each step the most informative linear direction is
identified via classifier-weight SVD, projected out, and a fresh logistic regression
is re-trained on the residual.
Table~\ref{tab:erasure} reports the signal erasure point --- the dimension at which
accuracy reaches the 6-class chance baseline of 16.7\% --- for each LLM variant.
...
\midrule
MPNet-FT-Final   & 98.14 & dim 15 \\
BGE-FT-Final     & 98.14 & dim 20 \\
BGE-Base-Final   & 95.17 & dim 26 \\
MPNet-Base-Final & 95.17 & dim 67 \\
\bottomrule
\end{tabular}
\label{tab:erasure}
\end{center}

Fine-tuned models achieve higher baseline accuracy but pack all emotional signal into
15--20 directions; once removed, signal collapses to chance entirely.
Pretrained MPNet-Base distributes signal across 67 dimensions --- a $4.5\times$ wider
manifold (Figure~\ref{fig:signal-cliff}).

\textbf{Cross-system extension.}
We extend the ablation to human brain fMRI and a fine-tuned Qwen model~\citep{qwen2025}
(Figure~\ref{fig:context-retention}).
The signal half-life $D_{50}$ --- the number of dominant directions whose removal halves
classification accuracy --- reveals qualitatively distinct encoding strategies across
systems.
The brain ($D_{50}\!=\!4$, AUC\,=\,0.302, baseline 55.0\%) uses a highly distributed
representation: removing just 4 directions halves accuracy, yet the curve then flattens
into a distributed plateau rather than dropping to chance~\citep{haxby2001}.
The fine-tuned Qwen model ($D_{50}\!=\!12$, AUC\,=\,0.260, baseline 97.8\%) concentrates
signal sharply with a steep cliff.
MPNet-Pretrained ($D_{50}\!=\!17$, AUC\,=\,0.331, baseline 95.6\%) distributes signal more
broadly, closer in profile to the brain than to the fine-tuned LLM.

\textbf{Concentration of emotion signal in embedding capacity.}
The ablation results motivate a direct analysis of how much of the LLM's 768-dimensional
embedding space is actually used for emotion.
Projecting into the top 20 SVD directions --- $\approx\!2.6\%$ of embedding capacity ---
resolves the apparent cloud structure of pretrained models dramatically, and accuracy
in this 20D subspace matches or exceeds full 768D across all architectures
(Figure~\ref{fig:silhouette}).

\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\textwidth]{silhouette_comparison_20D}
  \caption{\textbf{The Low-Rank Emotion Subspace.}
           ...
           Accuracy in 20D equals or exceeds 768D in all four variants.}
...
\begin{tabular}{lcccc}
\toprule
Variant & 768D Silhouette & 20D Silhouette & 768D Acc.\ (\%) & 20D Acc.\ (\%) \\
\midrule
MPNet-FT-Final   & 0.860 & 0.907 & 98.24 & 98.27 \\
BGE-FT-Final     & 0.685 & 0.835 & 97.99 & 98.09 \\
MPNet-Base-Final & 0.047 & 0.408 & 94.08 & 95.64 \\
BGE-Base-Final   & 0.057 & 0.415 & 93.78 & 95.12 \\
\bottomrule
\end{tabular}
\end{center}

For pretrained models the silhouette score rises from near-zero to $\approx\!0.41$ ---
discernible but overlapping clusters, indicating structured geometry obscured by
high-dimensional noise rather than absent.
The remaining 748 dimensions contribute only noise that marginally suppresses linear
classification.
Emotion is therefore a low-rank linear property: the apparent cloud in 768D is not an
absence of structure, but $\approx\!748$ noise dimensions overwhelming 20 signal dimensions.

% ---------------------------------------------------------------------------
\subsection{Finding 3: The Cross-System Ambiguity Gradient}
\label{sec:finding3}
% ---------------------------------------------------------------------------
...
\begin{figure}[t]
  \centering
  \begin{subfigure}[b]{0.42\textwidth}
    \includegraphics[width=\textwidth]{dist_logit_scatter_MPNet-FT-Final}
    \caption{LLM: centroid distance (geometric margin) vs.\ logit margin
             for MPNet-FT-Final ($r\!=\!0.988$).}
    \label{fig:dist-logit}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.55\textwidth}
    \includegraphics[width=\textwidth]{global_behavior_comparison}
    \caption{Cross-system: brain (solid red) vs.\ LLM (dashed blue) density profiles
             (left) and ambiguity gradients (right).
             Shape correlation $r\!=\!0.9565$ over 100 normalised-distance bins.}
    \label{fig:global-comparison}
  \end{subfigure}
  \caption{\textbf{The cross-system ambiguity gradient.}
           Both systems link geometric proximity to prototypes with representational
           certainty, through complementary mechanisms.}
  \label{fig:finding5}
\end{figure}
...
\textbf{LLMs: centroid distance $\to$ logit confidence.}
For fine-tuned LLMs, centroid distance is a strong predictor of logit confidence
...
\begin{tabular}{lcc}
\toprule
Variant & Dist-logit Pearson $r$ & Logit agreement in overlap (\%) \\
\midrule
...
The near-linear relationship ($r\!=\!0.9884$ for MPNet-FT) means geometric position
almost perfectly determines classifier confidence.
...
The $r\!=\!0.9565$ ($p\!=\!3.20\!\times\!10^{-54}$, 95\% CI [0.9370, 0.9725]) is
therefore computed over $N\!=\!100$ interpolated grid points, \emph{not} over 5
independent emotion categories or 200 biological subjects.
The tightly specified $p$-value reflects the effective sample size of 100 interpolation
bins; inference must rest on the permutation test ($n\!=\!5000$, $p\!=\!0.000$, which
destroys cross-system shape alignment when shuffled) and on the qualitative consistency
of the functional form (Figure~\ref{fig:global-comparison}).
Additional evidence: KS statistic $= 0.57$ ($p\!=\!2.46\!\times\!10^{-15}$), Cohen's
$d\!=\!1.34$ --- indicating that while gradient \emph{shape} is shared, absolute
density of representations around centroids differs substantially between systems.

% ---------------------------------------------------------------------------
\subsection{Finding 4: The Valence-Arousal Inversion}
\label{sec:inversion}
% ---------------------------------------------------------------------------
...
\begin{figure}[t]
  ...
  \vspace{0.5em}
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{centroid_map_brain_11d}
    \caption{Brain 11D centroid map (PCA): arousal-dominant organisation.
             Fear (high arousal) separates from sadness (low arousal);
             happiness clusters by positive valence+arousal.}
    \label{fig:centroid-brain}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{centroid_map_MPNet-Balanced}
    \caption{LLM centroid map (PCA): valence-dominant organisation.
             Happiness and fear separate on the horizontal axis (valence).}
    \label{fig:centroid-llm}
  \end{subfigure}
  \caption{\textbf{The Valence-Arousal Inversion.}
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \label{fig:rdm-llm}
  \end{subfigure}
  \caption{\textbf{RDM comparison.}
           Near-opposite pairwise geometry across systems (brain vs.\ fine-tuned LLM
...
\textbf{(6a) Valence-arousal axis alignment.}
...
\bottomrule
\end{tabular}
\caption{Global representational axis alignment (Figure~\ref{fig:va-inversion}).
         The axis priorities are swapped between LLMs and the brain.}
\label{tab:va-alignment}
\end{table}
...
\begin{tabular}{lccc}
\toprule
Emotion & Valence (1--9) & Arousal (1--9) & Quadrant \\
\midrule
Afraid / Fear      & 2.5 & 7.5 & negative, high arousal \\
Calm               & 7.0 & 2.5 & positive, low arousal \\
Delighted / Happin.& 8.2 & 6.5 & positive, moderate--high arousal \\
...
\bottomrule
\end{tabular}
\caption{External V-A reference coordinates (1--9 SAM scale).
         The quadrant structure --- not exact decimal values --- is the
         scientifically load-bearing element.}
\label{tab:va-coords}
\end{table}

\textbf{(6b) RDM comparison in 48D raw ROI space (Figure~\ref{fig:rdms}).}
...
The noise ceiling contextualises the divergence precisely: brain subjects agree with
each other at $r\!\approx\!0.47$, yet the fine-tuned LLM RDM is in the \emph{opposite
direction} ($r\!=\!-0.88$) from the brain RDM --- a magnitude substantially larger
than within-brain agreement.
Bootstrap 95\% CI: $[-0.9967,\,+0.6994]$ (wide, as expected with $K\!=\!3$, 1 df).
The reliable finding is the sign (consistently negative) and contrast with the positive
noise ceiling; specific magnitudes are indicative.

\textbf{(6c) RDM comparison in 11D systems-level space.}
...
Brain vs.\ pretrained LLM  & $-0.5476$ & $-0.9918$ \\
\bottomrule
\end{tabular}
\end{center}
...
In LLMs this operates via centroid distance predicting logit confidence; in the brain,
...
The near-opposite RDM correlation does not indicate that either system is wrong.
...
We have presented six findings characterising the geometry of emotion representations
in LLMs and human fMRI.
The core result is a paradox: a geometric competition principle for representational
ambiguity is observable across both systems --- LLMs via centroid distance predicting
logit confidence; the brain via centroid margin predicting classifier uncertainty ---
...
Overlap samples             & $\approx$73 (1.81\%) & $\approx$17 (0.42\%) \\
...
\midrule
MPNet-FT-Final   & 0.42  & 0.562 & $+1.875$ & $\approx$0.4375 \\
BGE-FT-Final     & 1.81  & 0.688 & $+1.750$ & $\approx$0.4375 \\
MPNet-Base-Final & 18.45 & 0.938 & $\phantom{+}0.000$ & $\approx$0.6875 \\
