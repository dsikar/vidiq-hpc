...
\begin{abstract}
% ===========================================================================
We investigate whether LLM embedding spaces and the human brain share a geometric
principle for representational ambiguity in emotion processing.
Across eight LLM variants (BGE and MPNet, pretrained and fine-tuned) applied to a
6-class text emotion dataset (4,038 validation samples) and brain fMRI from 40 subjects
(5 emotions, 48 ROIs; OpenNeuro DS005700), we identify six findings.
First, no embedding exists within 0.375--0.625 normalised units of any emotion centroid
(the \emph{Void}); density peaks at 0.56--0.94 units (the \emph{Belt}).
Second, overlap rates range from 0.42\% (fine-tuned) to 19.22\% (pretrained);
no embedding aligns exclusively with a single class prototype in any system.
Third, fine-tuned LLMs concentrate signal into 15--20 dimensions;
pretrained models distribute across 26--67.
Fourth, projecting to the top 20 SVD directions resolves pretrained cloud structure
(silhouette $+627$--$765\%$) while matching or exceeding 768D accuracy, confirming
emotion occupies $\approx\!2.6\%$ of embedding capacity.
Fifth, centroid margin predicts brain classification uncertainty
(AUC$\,{=}\,0.81$) while raw distance fails (AUC$\,{=}\,0.56$);
in fine-tuned LLMs, centroid distance predicts logit confidence ($r=0.957$--$0.988$).
Sixth, LLMs sort emotions by valence (PC1 $r=0.97$--$0.98$);
the brain sorts by arousal (PC1 $r=0.96$), producing near-opposite
dissimilarity matrices ($r=-0.88$) that deepen as biological noise is reduced.
Shared local geometry coexists with fundamentally different global representational priorities.
\end{abstract}
...
We present six findings in order of increasing cross-system scope.
Findings 1--4 characterise LLM geometry; Finding 5 introduces the ambiguity gradient;
Finding 6 documents the global representational inversion.
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.44\textwidth}
    ...
    \caption{PCA projections of all 8 LLM variants.
             Pretrained models (rows 1--2): overlapping clouds with no visible
             cluster structure.
             Fine-tuned models (rows 3--4): compact, well-separated islands.}
    \label{fig:clusters}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.53\textwidth}
    \includegraphics[width=\textwidth]{global_density_ambiguity_curves}
    \caption{Radial density profiles (top) and ambiguity gradients (bottom)
             for all 8 variants.
             The void region (zero density near centroid) and the Belt
             (peak density shell) are consistent across all systems.}
    \label{fig:density-ambiguity}
  \end{subfigure}
  \caption{\textbf{Cluster geometry and density structure (Findings 1 and 2).}
           (a) The cloud-to-island transition from pretrained to fine-tuned is
           visible in raw PCA projection.
           (b) Despite visual differences, all variants share the same two-zone
           radial density structure in normalised-distance space.}
  \label{fig:clusters-density}
\end{figure}

A consistent two-zone density structure emerges across all 8 LLM variants
(Figure~\ref{fig:clusters-density}).
...
\begin{tabular}{lcccc}
\toprule
Variant & Density Peak & Overlap Onset & Certainty Buffer & Void Onset \\
\midrule
MPNet-FT-Final  & 0.562 & 2.438 & $+1.875$ & 0.4375 \\
BGE-FT-Final    & 0.688 & 2.438 & $+1.750$ & 0.4375 \\
MPNet-Base-Final& 0.938 & 0.938 & $\phantom{+}0.000$ & 0.6875 \\
BGE-Base-Final  & 0.938 & 1.062 & $+0.125$ & 0.6875 \\
\bottomrule
\end{tabular}
\end{center}
...
% ---------------------------------------------------------------------------
\subsection{Finding 2: Unoccupied Centroids}
\label{sec:finding2}
% ---------------------------------------------------------------------------

No observed sample meets the condition of being within the void region near its own
prototype. \textbf{No embedding aligns exclusively with a single class prototype in
any tested model or the brain fMRI system.}

\textbf{Graduated overlap rates (final layer, full validation set):}
...
The most entangled pair is Happiness vs.\ Love (42\% pairwise overlap in pretrained
models), sharing adjacent circumplex positions. The most separable is Fear vs.\ Anger
(12\% overlap), despite both occupying the negative high-arousal quadrant.

% ---------------------------------------------------------------------------
\subsection{Finding 3: Information Compaction vs.\ Distribution}
\label{sec:finding3}
% ---------------------------------------------------------------------------
...
\begin{figure}[t]
  ...
  \end{subfigure}
  \caption{\textbf{Signal structure across systems.}
...
A separate iterative ablation compared three systems
...
\begin{tabular}{lccc}
\toprule
System & AUC & $D_{50}$ & Signal profile \\
\midrule
Human Brain                      & 0.302 & 4  & Highly distributed \\
MPNet-Pretrained                 & 0.331 & 17 & Redundant manifold \\
Qwen-FT~\citep{qwen2025}         & 0.260 & 12 & Compressed cliff   \\
\bottomrule
\end{tabular}
\end{center}

The brain's $D_{50}\!=\!4$ means removing just 4 dominant directions halves
classification accuracy, yet the curve flattens into a distributed plateau rather than
dropping to chance~\citep{haxby2001}, consistent with distributed biological coding.

% ---------------------------------------------------------------------------
\subsection{Finding 4: The Low-Rank Emotion Subspace}
\label{sec:finding4}
% ---------------------------------------------------------------------------
...
\textbf{Silhouette scores before and after projection into the top 20 SVD directions
...
\begin{tabular}{lccc}
\toprule
Variant & 768D Silhouette & 20D Silhouette & Relative Change \\
\midrule
MPNet-FT-Final   & 0.860 & 0.907 & $+5.5\%$ \\
BGE-FT-Final     & 0.685 & 0.835 & $+22.0\%$ \\
MPNet-Base-Final & 0.047 & 0.408 & $+765\%$ \\
BGE-Base-Final   & 0.057 & 0.415 & $+627\%$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Classification accuracy: 20D equals or exceeds 768D across all variants:}
...
% ---------------------------------------------------------------------------
\subsection{Finding 5: The Cross-System Ambiguity Gradient}
\label{sec:finding5}
% ---------------------------------------------------------------------------
...
% ---------------------------------------------------------------------------
\subsection{Finding 6: The Valence-Arousal Inversion}
\label{sec:inversion}
% ---------------------------------------------------------------------------
