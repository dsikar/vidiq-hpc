...
- `repo_context/paper_versions/neuroemo_neurips_full_submission_draft.pdf` — full NeurIPS submission draft generated by a separate AI client (pre-Claude). **Workflow:** A second draft will be generated using Claude agents from the compass and source data. The two drafts will then be compared and the best of both merged into the final submission. Do not treat either draft as authoritative until the merge review is complete.
...
- [ ] **Generate Claude-agent draft** — use compass + source data to produce a second full NeurIPS draft via Claude agents
- [ ] **Compare and merge drafts** — compare Claude draft against `repo_context/paper_versions/neuroemo_neurips_full_submission_draft.pdf` (existing draft from separate AI client); merge the best of both into the final submission
