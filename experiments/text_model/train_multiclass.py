...
def load_texts_and_labels(base_path: Path) -> tuple[list[str], np.ndarray]:
...
def save_evaluation_artifacts(run_config: RunConfig, eval_output: dict[str, object]) -> dict[str, object]:
    run_config.embeddings_dir.mkdir(parents=True, exist_ok=True)
...
def create_run_config(base_root: Path, run_name: str) -> RunConfig:
...
def save_checkpoint(model: EmotionEmbeddingClassifier, run_config: RunConfig) -> None:
    run_config.model_dir.mkdir(parents=True, exist_ok=True)
    model.transformer.save_pretrained(run_config.model_dir)
    torch.save(model.projection.state_dict(), run_config.model_dir / "head.pt")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train qwen3-based emotion classifier and log 768D embeddings.")
    parser.add_argument("--data-root", type=Path, required=True, help="Balanced dataset root containing artifacts and processed text/labels.")
...
def main() -> None:
    args = parse_args()
    set_seed(args.seed)
    dataset_root = args.data_root / "data" / "processed" / "train"
    texts, labels = load_texts_and_labels(dataset_root)
    ...
    run_name = args.run_name or f"qwen3-1.7B-{args.num_epochs}e-{args.batch_size}b-{random.randint(1000,9999)}"
    run_config = create_run_config(args.run_root, run_name)
    ...
    run_metrics_path = run_config.root / "train_metrics.json"
    with run_metrics_path.open("w", encoding="utf-8") as fh:
    ...
    tokenizer.save_pretrained(run_config.tokenizer_dir)
    print(f"Run completed. metrics={metrics}. artifacts stored under {run_config.root}")
